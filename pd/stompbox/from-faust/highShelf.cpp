...
class mydsp : public dsp {
  ...
  public:
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		float 	fSlow0 = powf(10,(0.025f * fslider0));
		float 	fSlow1 = (fConst0 * max((float)0, fslider1));
		float 	fSlow2 = ((sqrtf(fSlow0) * sinf(fSlow1)) / max(0.001f, fslider2));
		float 	fSlow3 = cosf(fSlow1);
		float 	fSlow4 = ((fSlow0 - 1) * fSlow3);
		float 	fSlow5 = (1.0f / ((1 + (fSlow0 + fSlow2)) - fSlow4));
		float 	fSlow6 = (1 + fSlow0);
		float 	fSlow7 = (0 - ((fSlow4 + fSlow2) - fSlow6));
		float 	fSlow8 = (fSlow0 + fSlow4);
		float 	fSlow9 = (fSlow0 * ((1 + fSlow8) - fSlow2));
		float 	fSlow10 = (fSlow6 * fSlow3);
		float 	fSlow11 = (2 * (0 - ((1 + fSlow10) - fSlow0)));
		float 	fSlow12 = (fSlow0 * (1 + (fSlow2 + fSlow8)));
		float 	fSlow13 = ((0 - (2 * fSlow0)) * ((fSlow0 + fSlow10) - 1));
		FAUSTFLOAT* input0 = input[0];
		FAUSTFLOAT* output0 = output[0];
		for (int i=0; i<count; i++) {
			fRec0[0] = (fSlow5 * (fSlow7 - ((fSlow9 * fRec0[1]) + (fSlow11 * fRec0[2]))));
			output0[i] = (FAUSTFLOAT)(((float)input0[i] * fRec0[0]) + (fSlow5 * ((fSlow12 * fRec0[1]) + (fSlow13 * fRec0[2]))));
			// post processing
