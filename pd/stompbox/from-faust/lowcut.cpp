...
class mydsp : public dsp {
  ...
  public:
	...
	virtual void buildUserInterface(UI* interface) {
		...
		interface->addHorizontalSlider("attenuation", &fslider1, 0.0f, -96.0f, 1e+01f, 0.1f);
		interface->declare(&fslider0, "style", "knob");
		interface->declare(&fslider0, "unit", "Hz");
		interface->addHorizontalSlider("freq", &fslider0, 1e+02f, 2e+01f, 5e+03f, 1.0f);
		interface->closeBox();
	}
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		float 	fSlow0 = tanf((fConst0 * fslider0));
		float 	fSlow1 = powf(10,(0 - (0.05f * fslider1)));
		float 	fSlow2 = sqrtf((2 * fSlow1));
		float 	fSlow3 = (fSlow0 * fSlow1);
		float 	fSlow4 = (1.0f / (1 + (fSlow0 * (fSlow2 + fSlow3))));
		float 	fSlow5 = (1 + (fSlow0 * (1.4142135623730951f + fSlow0)));
		float 	fSlow6 = (1 + (fSlow0 * (fSlow3 - fSlow2)));
		float 	fSlow7 = faustpower<2>(fSlow0);
		float 	fSlow8 = (2 * ((fSlow7 * fSlow1) - 1));
		float 	fSlow9 = (2 * (fSlow7 - 1));
		float 	fSlow10 = (1 + (fSlow0 * (fSlow0 - 1.4142135623730951f)));
		FAUSTFLOAT* input0 = input[0];
		FAUSTFLOAT* output0 = output[0];
		for (int i=0; i<count; i++) {
			fRec0[0] = ((float)input0[i] - (fSlow4 * ((fSlow6 * fRec0[2]) + (fSlow8 * fRec0[1]))));
			output0[i] = (FAUSTFLOAT)(fSlow4 * (((fSlow5 * fRec0[0]) + (fSlow9 * fRec0[1])) + (fSlow10 * fRec0[2])));
			// post processing
