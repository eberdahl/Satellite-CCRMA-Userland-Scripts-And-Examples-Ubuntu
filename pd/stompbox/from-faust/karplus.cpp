...
class mydsp : public dsp {
  ...
  public:
	...
	virtual void buildUserInterface(UI* interface) {
		...
		interface->declare(&fslider2, "unit", "f");
		interface->addHorizontalSlider("excitation", &fslider2, 128.0f, 2.0f, 512.0f, 1.0f);
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		float 	fSlow1 = (4.656612875245797e-10f * fslider1);
		...
		for (int i=0; i<count; i++) {
			...
			fVec0[0] = fSlow2;
			fRec2[0] = ((fRec2[1] + ((fSlow2 - fVec0[1]) > 0.0f)) - (fSlow3 * (fRec2[1] > 0.0f)));
			fVec1[IOTA&511] = ((fSlow0 * (fRec0[1] + fRec0[2])) + (fSlow1 * (iRec1[0] * (fRec2[0] > 0.0f))));
