  ...
  public:
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		for (int i=0; i<count; i++) {
			float fTemp0 = (fSlow4 * fRec0[2]);
			fRec0[0] = ((fSlow2 * (fSlow3 - fTemp0)) - fRec0[1]);
			output0[i] = (FAUSTFLOAT)(((float)input0[i] * fRec0[0]) + (fSlow2 * (fTemp0 + (fSlow3 * fRec0[1]))));
			// post processing
