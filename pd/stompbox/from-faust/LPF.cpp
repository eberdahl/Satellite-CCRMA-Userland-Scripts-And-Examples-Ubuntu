...
class mydsp : public dsp {
  ...
  public:
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		float 	fSlow5 = cosf(fSlow0);
		float 	fSlow6 = (0 - (2 * fSlow5));
		float 	fSlow7 = (1 - fSlow5);
		float 	fSlow8 = (0.5f * fSlow7);
		float 	fSlow9 = (fSlow7 / fSlow2);
		FAUSTFLOAT* input0 = input[0];
		FAUSTFLOAT* output0 = output[0];
		for (int i=0; i<count; i++) {
			fRec0[0] = (fSlow3 * (fSlow4 - ((fSlow6 * fRec0[2]) + (fSlow8 * fRec0[1]))));
			output0[i] = (FAUSTFLOAT)(((float)input0[i] * fRec0[0]) + (fSlow9 * (fRec0[2] + (0.5f * fRec0[1]))));
			// post processing
