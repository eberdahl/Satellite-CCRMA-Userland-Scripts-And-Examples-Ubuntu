...
class mydsp : public dsp {
  ...
  public:
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		float 	fSlow1 = (0.5f * (sinf(fSlow0) / max(0.001f, fslider1)));
		float 	fSlow2 = (1.0f / (1 + fSlow1));
		float 	fSlow3 = (1 - fSlow1);
		float 	fSlow4 = cosf(fSlow0);
		float 	fSlow5 = (0 - (2 * fSlow4));
		...
		for (int i=0; i<count; i++) {
			float fTemp0 = (fSlow7 * fRec0[1]);
			fRec0[0] = (fSlow2 * (fSlow3 - ((fSlow5 * fRec0[2]) + fTemp0)));
			output0[i] = (FAUSTFLOAT)(((float)input0[i] * fRec0[0]) + (fSlow2 * (fTemp0 + (fSlow8 * fRec0[2]))));
			// post processing
