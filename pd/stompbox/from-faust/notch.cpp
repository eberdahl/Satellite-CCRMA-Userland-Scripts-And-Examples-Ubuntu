...
class mydsp : public dsp {
  ...
  public:
	...
	virtual void buildUserInterface(UI* interface) {
		interface->openVerticalBox("notch");
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		for (int i=0; i<count; i++) {
			float fTemp0 = (fRec0[1] + (fSlow4 * fRec0[2]));
			fRec0[0] = (fSlow2 * (fSlow3 - fTemp0));
			output0[i] = (FAUSTFLOAT)(((float)input0[i] * fRec0[0]) + (fSlow2 * fTemp0));
			// post processing
