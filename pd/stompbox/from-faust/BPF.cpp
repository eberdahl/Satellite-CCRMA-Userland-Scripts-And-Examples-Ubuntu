...
class mydsp : public dsp {
  ...
  public:
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		float 	fSlow7 = (0 - fSlow3);
		float 	fSlow8 = (0 - (2 * cosf(fSlow0)));
		float 	fSlow9 = (0.5f * (fSlow1 / (fSlow2 * fSlow4)));
		FAUSTFLOAT* input0 = input[0];
		FAUSTFLOAT* output0 = output[0];
		for (int i=0; i<count; i++) {
			fRec0[0] = (fSlow5 * (fSlow6 - ((fSlow7 * fRec0[1]) + (fSlow8 * fRec0[2]))));
			output0[i] = (FAUSTFLOAT)(((float)input0[i] * fRec0[0]) + (fSlow9 * fRec0[1]));
			// post processing
