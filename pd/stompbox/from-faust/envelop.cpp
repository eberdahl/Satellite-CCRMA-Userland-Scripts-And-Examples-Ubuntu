...
class mydsp : public dsp {
  ...
  public:
	...
	virtual void buildUserInterface(UI* interface) {
		...
		interface->declare(&fentry0, "unit", "ms");
		interface->addNumEntry("attack", &fentry0, 2e+01f, 1.0f, 1e+03f, 1.0f);
		interface->declare(&fentry1, "2", "");
		interface->declare(&fentry1, "style", "knob");
		interface->addNumEntry("decay", &fentry1, 2.0f, 1.0f, 1e+02f, 0.1f);
		interface->declare(&fentry2, "3", "");
		interface->declare(&fentry2, "style", "knob");
		interface->declare(&fentry2, "unit", "pc");
		interface->addNumEntry("sustain", &fentry2, 1e+01f, 1.0f, 1e+02f, 0.1f);
		interface->declare(&fentry3, "4", "");
		interface->declare(&fentry3, "style", "knob");
		interface->addNumEntry("release", &fentry3, 1e+01f, 1.0f, 1e+02f, 0.1f);
		interface->closeBox();
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		int 	iSlow1 = (fSlow0 > 0);
		float 	fSlow2 = (fConst0 / fentry0);
		float 	fSlow3 = (1e-05f * fentry1);
		float 	fSlow4 = (0.01f * fentry2);
		float 	fSlow5 = (1e-05f * fentry3);
		int 	iSlow6 = (fSlow0 <= 0);
		FAUSTFLOAT* output0 = output[0];
		for (int i=0; i<count; i++) {
			iRec0[0] = (iSlow1 & (iRec0[1] | (fRec1[1] >= 1)));
			fRec1[0] = ((fSlow2 * (((iRec0[1] == 0) & iSlow1) & (fRec1[1] < 1))) + (fRec1[1] * ((1 - (fSlow3 * (iRec0[1] & (fRec1[1] > fSlow4)))) - (fSlow5 * (iSlow6 & (fRec1[1] > 0))))));
			iRec2[0] = (12345 + (1103515245 * iRec2[1]));
			output0[i] = (FAUSTFLOAT)(4.656612875245797e-10f * (fRec1[0] * iRec2[0]));
			// post processing
			iRec2[1] = iRec2[0];
			fRec1[1] = fRec1[0];
			iRec0[1] = iRec0[0];
