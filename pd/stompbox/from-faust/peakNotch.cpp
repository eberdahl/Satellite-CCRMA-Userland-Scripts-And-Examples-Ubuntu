...
class mydsp : public dsp {
  ...
  public:
	...
	virtual void buildUserInterface(UI* interface) {
		interface->openVerticalBox("peakNotch");
		interface->addHorizontalSlider("Freq", &fslider0, 1e+03f, 1e+02f, 1e+04f, 1.0f);
		interface->declare(&fslider2, "unit", "lin");
		interface->addHorizontalSlider("Gain", &fslider2, 1.0f, 0.0f, 8.0f, 0.01f);
		interface->addHorizontalSlider("Q", &fslider1, 1.0f, 0.01f, 1e+02f, 0.01f);
		interface->closeBox();
	}
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		float 	fSlow0 = (fConst0 * max((float)0, fslider0));
		float 	fSlow1 = sinf(fSlow0);
		float 	fSlow2 = max(0.001f, fslider1);
		float 	fSlow3 = sqrtf(max(1e-05f, fslider2));
		float 	fSlow4 = (0.5f * (fSlow1 / (fSlow2 * fSlow3)));
		float 	fSlow5 = (1.0f / (1 + fSlow4));
		float 	fSlow6 = (1 - fSlow4);
		float 	fSlow7 = (0.5f * ((fSlow3 * fSlow1) / fSlow2));
		float 	fSlow8 = (1 - fSlow7);
		float 	fSlow9 = (0 - (2 * cosf(fSlow0)));
		float 	fSlow10 = (1 + fSlow7);
		FAUSTFLOAT* input0 = input[0];
		FAUSTFLOAT* output0 = output[0];
		for (int i=0; i<count; i++) {
			float fTemp0 = (fSlow9 * fRec0[2]);
			fRec0[0] = (fSlow5 * (fSlow6 - ((fSlow8 * fRec0[1]) + fTemp0)));
			output0[i] = (FAUSTFLOAT)(((float)input0[i] * fRec0[0]) + (fSlow5 * (fTemp0 + (fSlow10 * fRec0[1]))));
			// post processing
